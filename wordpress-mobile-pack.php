...
function wordpress_mobile_pack_soft_copy($source_dir, $destination_dir) {
  if(file_exists($destination_dir)) {
    if (!is_writable($destination_dir) || !is_executable($destination_dir)) {
      update_option('wpmp_warning', __('<strong>Could not install files</strong> to ') . $destination_dir . ('. Please ensure that the web server has write- and execute-access to that directory.'));
      return;
    }
  ...
  while($source_file = readdir($dir_handle)) {
    ...
    if (file_exists($destination_child = "$destination_dir/$source_file")) {
      update_option('wpmp_flash', "<strong>A file already existed</strong> at $destination_dir/$source_file. It may have been installed by a previous version of this plugin and so it was not overwritten.<ul><li>You should remove the existing mobile theme folders if you want to fully reset or upgrade them.</li><li>Take precautions if you have manually edited any theme files - your changes should be re-applied.</li><li>De-activate and re-activate the plugin to complete the installation of the themes.</li></ul>");
      continue;
    }
